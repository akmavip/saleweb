...
<f:formatDate value="${prod.productDate}" pattern="dd-MM-yyyy"
	var="date" />
<f:formatNumber value="${prod.unitPrice}" pattern="#,### VND" var="price" />
<div class="panel panel-success nn-detail" data-id="${prod.id}">
	...
	<div class="panel-body">
		...
		<ul class="col-sm-7">
			...
			<li><strong>Giá</strong>: <em>${price}</em></li>
			...
			<li><strong>Ngày nhập</strong>: <em>${date}</em></li>
			<li><strong>Có sẵn</strong>: <em>${prod.available}</em></li>
			<li><strong>Loại sách</strong>: <em>${prod.category.name}</em></li>
		</ul>
	</div>
